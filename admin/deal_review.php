...
if(isset($_GET['deal'])){
  ...
  $stmt = $pdo->prepare("SELECT deal.deal_id,deal_name,deal_status,deal_company,country_listvalidity,validity, reward, reward_unit
    FROM deal_review JOIN deal ON deal_review.deal_id = deal.deal_id WHERE deal_status = 'pending' or deal_status='rejected'");
  $stmt->execute();
  ...
  if(isset($_POST['approve'])) {
  	...
  	if($comment!="") {
    $id = $_SESSION['dealID'];
    $stmt = $pdo->prepare("UPDATE deal_review SET deal_status ='approved',deal_comment='$comment' WHERE deal_id='$id'");
    $stmt->execute();
    $stmt = $pdo->prepare("UPDATE deal_page SET enabled ='1' WHERE deal_id='$id'");
    $stmt->execute();
    header('Location: deal_review.php');
  	} else {
  		echo "<p style='color:red;'>Please add a comment</p>";
...
if($comment!="") {
  ...
  $stmt = $pdo->prepare("UPDATE deal_review SET deal_status ='rejected',deal_comment='$comment' WHERE deal_id='$id'");
  $stmt->execute();
  header('Location: deal_review.php');
} else {
  		echo "<p style='color:red;'>Please add a comment</p>";
              ...
              <?php
                if($all){
                  foreach($rows as $row){
                    ...
                    echo '<div class="card-footer text-muted">Valid until '.htmlentities($row['validity']).' Status: '.htmlentities($row['deal_status']).'</div></div>';
                    echo '</a>';
                    echo '<br/>';
                  }
                } else {
                    ...
                    echo '<p class="text-start card-text d-flex align-items-center">Status: <br/>'.htmlentities($row['deal_status']).'</p>';
                    echo '<p class="text-start card-text d-flex align-items-center">Comment: <br/>'.htmlentities($row['deal_comment']).'</p></div>';
